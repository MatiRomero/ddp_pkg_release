...
and the `rbatch+` variant (both `+` policies scale shadow potentials by 0.5).
...
- Batch/batch+/rbatch/rbatch+ use critical-aware weights so for critical `i`: weight(i,j) = reward(i,j) - s_j
