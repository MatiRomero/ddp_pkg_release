...
* **Delimited/CSV files** must provide a header row with `type` and `mean_dual` (or `dual`) columns. Extra columns are ignored; missing or invalid values raise `ValueError`.【F:src/ddp/scripts/run.py†L54-L81】
