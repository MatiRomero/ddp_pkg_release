...
def run_instance(
    ...
    dispatch labels ``batch+`` and ``rbatch+`` apply their respective policies
    ...
    for sh in shadows:
        ...
        if "batch+" in dispatches:
            sp_plus = np.array(sp_base, dtype=float, copy=True)
            sp_plus = sp_plus * BATCH_PLUS_GAMMA + BATCH_PLUS_TAU
            extra_dispatch_state["batch+"] = (
...
def run_once(
    ...
    invokes the corresponding heuristic with fixed ``gamma = 0.5`` and
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help=(
            "Comma-separated dispatch policies. The 'batch+' and 'rbatch+' variants "
            "run BATCH/RBATCH with shadow potentials scaled by 0.5."
        ),
    )
